...
CSceneNode_AABB* CSceneNode_AABB::Create(CSceneNode** pChildNodeArr, UINT* iNumberingArr, UINT iArrSize, bool bIsTLAS, CGameObject* pContainingObj)
{
	CSceneNode_AABB* pInstance = new CSceneNode_AABB;

	pInstance->Set_ContainingObj(pContainingObj);

	if (FAILED(pInstance->Initialize(pChildNodeArr, iNumberingArr, iArrSize, bIsTLAS)))
	{
...
HRESULT CSceneNode_AABB::Initialize(CSceneNode** pChildNodeArr, UINT* iNumberingArr, UINT iChildArrSize, bool bIsTLAS)
{
	HRESULT hr = S_OK;

	hr = CSceneNode::Initialize(pChildNodeArr, iNumberingArr, iChildArrSize, bIsTLAS);
