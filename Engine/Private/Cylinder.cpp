...
CCylinder* CCylinder::Create(const wstring& strGridMeshTag)
{
	CCylinder* pInstance = new CCylinder;
	if (strGridMeshTag.empty())
	{
		MSG_BOX("CCylinder : Enter Grid Tag");
		return nullptr;
	}
	else
	{
		pInstance->m_strCylinderMeshTag = strGridMeshTag;
	}
...
HRESULT CCylinder::Initialize(void* pArg)
{
	...
	if (FAILED(hr)) return hr;*/
	MESHOBJ_INIT_DESC meshObjDesc{ true, m_strCylinderMeshTag };
...
void CCylinder::Tick(_float fDeltaTime)
{
	int a = 0;
}

void CCylinder::Late_Tick(_float fDeltaTime)
{
	int a = 0;
}
...
void CCylinder::Render(ID3D12GraphicsCommandList* pCmdList, FrameResource* pFrameResource)
{
	// Update CB
...
Matrix CCylinder::Get_WorldMatrix()
{
	return m_pTransformCom->WorldMatrix();
}

Vector3 CCylinder::Get_Pos()
{
	return m_pTransformCom->Position();
}

Vector3 CCylinder::Get_ScaleXYZ()
{
	return CGameObject::Get_ScaleXYZ();
}

void CCylinder::Set_Position(const Vector3& vPos)
{
	m_pTransformCom->Set_Position(vPos);
}

void CCylinder::Set_Scale(const Vector3& vScale)
{
	m_pTransformCom->Set_Scale(vScale);
}

MATERIAL_INFO CCylinder::Get_MaterialInfo()
{
